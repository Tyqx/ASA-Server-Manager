...
public partial class ServerPage : System.Windows.Controls.UserControl
{
    ...
    // ASA SERVER MANAGER
    // =========================================================
    // MainWindow uses this property to access the manager
    // belonging to this specific server page.
    // =========================================================

    public AsaServerManager AsaServerManager =>
        _asaServerManager;

    // Keep this alias too so any existing ServerPage code
    // using ServerManager continues to work.
    public AsaServerManager ServerManager =>
        _asaServerManager;
    ...
    private readonly ServerBackupManager _backupManager =
        new ServerBackupManager();
    ...
    string text)
{
    ...
    ConsoleTextBox.ScrollToEnd();
        ...
        // Backups are stored at the default location computed by
        // ServerBackupManager (a drive-root "ASA-Backups" folder
        // named after the server). Pass a non-empty path here if
        // a custom backup location is ever exposed in the UI.
...
private int _backupRetentionCount = 10;
       private string BackupDirectorySetting
{
    get
    {
        if (!string.IsNullOrWhiteSpace(
        ...
        {
            return BackupFolderTextBox.Text.Trim();
        }
...
private void ConfigureBackupTimer()
{
    ...
    if (string.IsNullOrWhiteSpace(BackupFolderTextBox?.Text))
        return;
    ...
    _backupTimer = new System.Threading.Timer(
        async _ =>
        {
            await PerformAutomaticBackupAsync();
        },
        null,
...
private void StopBackupTimer()
{
    ...
    {
        // Timer cleanup should never crash the page.
    }
...
private async Task PerformAutomaticBackupAsync()
{
    ...
    {
        string serverPath =
            ServerPathTextBox?.Text?.Trim() ?? string.Empty;

        string backupDirectory =
            BackupFolderTextBox?.Text?.Trim() ?? string.Empty;
    ...
    {
        // Automatic backups must never crash the server page.
    }
}
...
private void EnforceBackupRetention()
{
    try
    {
        ...
        _backupRetentionCount = retention;
        ...
        IEnumerable<BackupInfo> backupsToDelete =
            backups
        ...
        {
            _backupManager.DeleteBackup(backup);
        }
    ...
    {
        // Retention cleanup should never crash the page.
    }
}
private int GetSelectedBackupInterval()
{
    try
    {
...
private void BackupSettingsChanged(
    object sender,
    RoutedEventArgs e)
{
    if (BackupEnabledCheckBox == null)
        return;

    _automaticBackupsEnabled =
        BackupEnabledCheckBox.IsChecked == true;

    bool enabled = _automaticBackupsEnabled;

    BackupIntervalComboBox.IsEnabled = enabled;
    BackupRetentionTextBox.IsEnabled = enabled;

    ConfigureBackupTimer();
}


private void BackupIntervalComboBox_SelectionChanged(
    object sender,
    SelectionChangedEventArgs e)
{
    if (!IsLoaded)
    ...
    if (BackupEnabledCheckBox?.IsChecked != true)
        return;
...
private void BackupRetentionTextBox_TextChanged(
    object sender,
    TextChangedEventArgs e)
{
    if (!int.TryParse(
...
private void BrowseBackupFolderButton_Click(
    object sender,
    RoutedEventArgs e)
{
    try
    {
        ...
        if (!string.IsNullOrWhiteSpace(
                BackupFolderTextBox?.Text) &&
            Directory.Exists(BackupFolderTextBox.Text))
        {
    ...
    {
        MessageBox.Show(
            $"Unable to select the backup folder.\n\n{ex.Message}",
            "Backup Folder",
            MessageBoxButton.OK,
            MessageBoxImage.Error);
    }
        ...
        private void RefreshBackups()
        {
            try
            {
                ...
                {
                    NoBackupsText.Visibility =
                        hasBackups
                            ? Visibility.Collapsed
                            : Visibility.Visible;
                }

                if (BackupsListBox != null)
                {
                    BackupsListBox.Visibility =
                        hasBackups
                            ? Visibility.Visible
                            : Visibility.Collapsed;
                }
            ...
            {
                // Refreshing the list should never crash the page.
            }
        }
    ...
    RoutedEventArgs e)
{
    ...
    {
        MessageBox.Show(
            "Set the server folder before creating a backup.",
            "Backup",
            MessageBoxButton.OK,
            MessageBoxImage.Warning);
    ...
    {
        MessageBox.Show(
            "Select a backup folder before creating a backup.",
            "Backup",
            MessageBoxButton.OK,
            MessageBoxImage.Warning);
    ...
    try
    {
        ...
        {
            MessageBox.Show(
                "Backup failed. Check that the server folder and backup folder exist.",
                "Backup",
                MessageBoxButton.OK,
                MessageBoxImage.Error);
    ...
    RoutedEventArgs e)
{
    if (sender is not Button button ||
        button.Tag is not BackupInfo backup)
    {
        return;
    }
    ...
    MessageBoxResult confirm =
        MessageBox.Show(
            $"Restore \"{backup.Name}\"?\n\n" +
            "This will overwrite the server's current save data. " +
